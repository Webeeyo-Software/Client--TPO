...
const AddressDetailsScreen: React.FC = () => {
  ...
  const handleChange = (field: keyof AddressDetailsProps, value: string | boolean) => {
    setAddressDetails((prev) => ({
      ...prev,
      [field]: value,
      ...(field === 'sameAsLocal' && value === true ? { permanentAddress: prev.localAddress } : {}),
    }));
  };
  ...
  const fetchAddressDetails = async () => {
    setLoading(true);
    try {
      const { data } = await api.get<AddressDetailsProps>(`/api/AddressDetails/${registrationNo}`);

      if (data) {
        setAddressDetails({
          localAddress: data.localAddress || '',
          permanentAddress: data.permanentAddress || '',
          sameAsLocal: data.permanentAddress === data.localAddress,
        ...
        Alert.alert('Info', 'No address details found.');
      }
    } catch (error) {
      console.error('[Fetch Address Details] Error:', error);
  ...
  const saveAddressDetails = async () => {
    ...
    try {
      ...
      const { data } = await api.put(`/api/AddressDetails/${registrationNo}`, payload);

      if (data && data.message) {
        Alert.alert('Success', data.message);
      } else {
        Alert.alert('Error', 'Failed to update address details.');
      }
    } catch (error) {
  ...
  return (
    <View className="flex-1 bg-white px-4 pt-12">
      ...
      <ScrollView className="p-4">
        ...
        <PrimaryButton label="Save" onPress={saveAddressDetails} />
      </ScrollView>
    </View>
  );
};
